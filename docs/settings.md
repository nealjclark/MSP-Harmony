...
Cost synchronization keeps an independent checkpoint for every subscription and for each daily or monthly stream in `azure_cost_sync_checkpoints`. Daily collection advances through the previous Eastern calendar day and re-queries a five-day correction window so Azure's late cost revisions replace existing `azure_cost_daily` rows. A successful response advances coverage even when Azure returns zero rows. Re-running after a subscription is already current skips its cost, inventory, and Advisor calls. Completed-month backfill uses `AZURE_MONTHLY_BACKFILL_MONTHS`, persists a monthly cursor, and processes the same month across subscriptions before moving to the next month. Failed requests do not advance coverage; throttled requests record their next retry time, and tenant/client/QPU throttling defers the remaining shared queue.
