import { AlertTriangle, Download, FileSpreadsheet, Package, RefreshCcw, Search } from 'lucide-react';
      ...
      <div className="software-inventory-layout">
        <section className="work-surface report-surface software-inventory-results">
          {!selectedReport ? (
            ...
            <>
              <header className="surface-header software-inventory-header">
                <div>
                  <span className="section-kicker">{selectedReport.siteName ? 'Site report' : 'Customer report'}</span>
                  <h2>{selectedReport.customerName}{selectedReport.siteName ? ` / ${selectedReport.siteName}` : ' / All sites'}</h2>
                  <p>Requested by {selectedReport.requestedBy} on {formatDateTime(selectedReport.requestedAt)}</p>
                </div>
                <div className="software-inventory-report-actions">
                  <span className={`status-pill ${reportStatusClass(selectedReport.status)}`}>{reportStatusLabel(selectedReport.status)}</span>
                  <button className="button secondary compact" disabled={!reportReady || downloading} onClick={() => void downloadReport()} type="button">
                    <Download size={16} /> {downloading ? 'Preparing' : 'Excel'}
                  </button>
                </div>
              </header>
              ...
              {reportReady ? (
                <>
                  <div className="software-inventory-summary">
                    <article><span>Devices collected</span><strong>{selectedReport.completedDevices.toLocaleString()}</strong></article>
                    <article><span>Device failures</span><strong>{selectedReport.failedDevices.toLocaleString()}</strong></article>
                    <article><span>Application rows</span><strong>{selectedReport.applicationCount.toLocaleString()}</strong></article>
                    <article><span>Retained until</span><strong>{formatDate(selectedReport.expiresAt)}</strong></article>
                  </div>
                  ...
                  <div className="software-inventory-table-scroll">
                    {tab === 'counts' ? <CountsTable result={counts} loading={tableLoading} /> : <DetailsTable result={details} loading={tableLoading} />}
                  </div>
        ...
        <aside className="work-surface software-inventory-history" aria-label="Software inventory report history">
          <header className="surface-header"><div><span className="section-kicker">Saved for 90 days</span><h2>Report history</h2></div></header>
          <div className="software-inventory-history-list">
            {reports.length === 0 ? <p>No reports have been run yet.</p> : reports.map((report) => (
              <button className={selectedReport?.id === report.id ? 'active' : ''} key={report.id} onClick={() => void openReport(report.id)} type="button">
                <span><strong>{report.customerName}</strong><small>{report.siteName ?? 'All sites'}</small></span>
                <span><em className={reportStatusClass(report.status)}>{reportStatusLabel(report.status)}</em><time>{formatDateTime(report.requestedAt)}</time></span>
              </button>
            ))}
          </div>
        </aside>
...
function CountsTable(props: { result: PagedResult<SoftwareInventoryCountRow> | null; loading: boolean }) {
  ...
  return (
    <table className="software-inventory-table counts">
      <thead><tr><th>Software</th><th>Devices</th><th>Installations</th><th>Publishers</th><th>Versions</th></tr></thead>
      <tbody>{props.result.rows.map((row) => (
        <tr key={row.applicationName.toLowerCase()}><td><strong>{row.applicationName}</strong></td><td>{row.deviceCount.toLocaleString()}</td><td>{row.installationCount.toLocaleString()}</td><td>{row.publishers.join(', ') || '—'}</td><td>{row.versions.join(', ') || '—'}</td></tr>
      ))}</tbody>
    </table>
  );
}
...
function DetailsTable(props: { result: PagedResult<SoftwareInventoryDetailRow> | null; loading: boolean }) {
  ...
  return (
    <table className="software-inventory-table details">
      <thead><tr><th>Customer / site</th><th>Device</th><th>Class / last user</th><th>Software</th><th>Version</th><th>Publisher</th><th>Installed</th><th>Status</th></tr></thead>
      <tbody>{props.result.rows.map((row, index) => (
        <tr className={row.collectionStatus === 'Failed' ? 'failed' : ''} key={`${row.deviceId}:${row.applicationName ?? 'error'}:${row.version ?? ''}:${index}`}>
          ...
          <td>{row.applicationName ?? 'Collection failed'}{row.installLocation ? <small>{row.installLocation}</small> : null}</td>
          <td>{row.version ?? '—'}</td><td>{row.publisher ?? '—'}</td><td>{row.installDate ?? '—'}</td>
          <td><span className={`status-pill ${row.collectionStatus === 'Failed' ? 'blocked' : 'approved'}`}>{row.collectionStatus}</span>{row.collectionError ? <small className="software-inventory-row-error">{row.collectionError}</small> : null}</td>
        </tr>
      ))}</tbody>
    </table>
  );
...
function reportStatusLabel(status: SoftwareInventoryReport['status']) {
  if (status === 'partial') return 'Complete with warnings';
  return status.charAt(0).toUpperCase() + status.slice(1);
}

function reportStatusClass(status: SoftwareInventoryReport['status']) {
